...
vec4 normalCalc()
{
	...
	float currentHeight = texture2D(hf, texCoordIn.xy).r;

	vec3 heightPos = vec3(position.x, currentHeight, position.z);

	vec3 slopeX = vec3(position.x + off, hX, position.z) - heightPos;
	vec3 slopeZ = vec3(position.x, hZ, position.z + off) - heightPos;
